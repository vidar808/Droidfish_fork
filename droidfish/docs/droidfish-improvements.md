...
### 2. ~~Outdated First-Time User Experience~~ (DONE)
**Severity:** Medium (user impact)

The TourGuide overlay library has been removed and replaced with a simple welcome dialog that directs new users to the About screen and full manual. The tourguide library (6 Java files), tooltip layout, and all localized tour strings were cleaned up.
...
### ~~13. Engine Startup Time~~ (Partially Addressed)
**Severity:** Low
...
**Partial fix:** Built-in engines now load directly from `nativeLibraryDir` (no copy needed). Dead asset-checksum code was removed. External engines still use the size+timestamp copy check.
...
| 2 | Onboarding Refresh | Medium | Medium | P3 | **DONE** |
