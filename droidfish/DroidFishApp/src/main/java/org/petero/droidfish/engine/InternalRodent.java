...
public class InternalRodent extends ExternalEngine {
    ...
    protected void configureProcessBuilder(ProcessBuilder pb) {
        // Set environment variables so Rodent IV 0.33 finds its home,
        // personalities and books via absolute paths.
        File dataDir = new File(context.getFilesDir(), "rodent");
        dataDir.mkdirs();
        pb.directory(dataDir);
        pb.environment().put("RODENT4HOME",
                dataDir.getAbsolutePath() + "/");
        pb.environment().put("RODENT4PERSONALITIES",
                new File(dataDir, "personalities").getAbsolutePath());
        pb.environment().put("RODENT4BOOKS",
    ...
    private void copyDataFiles() throws IOException {
        ...
        // Copy guide.bin and guide_960.bin (Chess960 support)
        File guideFile = new File(dataDir, "guide.bin");
        if (!guideFile.exists()) {
            copyAssetFile("rodent/guide.bin", guideFile);
        }
        File guide960File = new File(dataDir, "guide_960.bin");
        if (!guide960File.exists()) {
            copyAssetFile("rodent/guide_960.bin", guide960File);
        }
