...
finish:
  // In infinite mode, wait for "stop" command before outputting bestmove.
  // The UCI loop sets thread_data.stop when "stop" is received.
  if (thread_info.thread_id == 0 && !thread_info.doing_datagen &&
      thread_info.is_infinite && !thread_data.stop) {
