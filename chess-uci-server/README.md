...
│   │   └── ...
│   └── relay/               # Relay server deployment (systemd, VPS)
...
Copy `example_config.json` to `config.json` to get started (the setup wizard and installer do this automatically). Edit `config.json` only when you need to:
