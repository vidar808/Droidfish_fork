...
void cEngine::Think(POS *p) {
    ...
    fprintf(stderr, "RODENT: Think() entered, p=%p\n", (void*)p);
    POS curr[1];
...
void cEngine::Iterate(POS *p, int *pv) {
    ...
    fprintf(stderr, "RODENT: Iterate() entered, maxDepth=%d\n", msSearchDepth);
    int cur_val = 0;
    int depthCounter = 0;
    ...
    for (mRootDepth = 1 + offset; mRootDepth <= msSearchDepth; mRootDepth++) {
        ...
        // (skip in infinite/analysis mode so analysis stays visible)

        if (mRootDepth >= 8 && mFlRootChoice == false && !Glob.isInfinite) {
            break;
        }

        // Abort search on finding checkmate score
        // (skip in infinite/analysis mode - keep searching to show stable PV)

        if (!Glob.isInfinite && (cur_val > MAX_EVAL || cur_val < -MAX_EVAL)) {
    ...
    // For correct exit from fixed depth search, signal abort.
    // In infinite mode, do NOT abort - wait for "stop" command.
    if (!Par.shut_up && !Glob.isInfinite) Glob.abortSearch = true;
}
