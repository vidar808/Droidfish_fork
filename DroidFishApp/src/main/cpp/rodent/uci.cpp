...
void UciLoop() {
    ...
    fprintf(stderr, "RODENT: UciLoop SetPosition...\n");
    p->SetPosition(START_POS);
    fprintf(stderr, "RODENT: UciLoop AllocTrans(16)...\n");
    Trans.AllocTrans(16);
    fprintf(stderr, "RODENT: UciLoop ready, waiting for commands\n");
    for (;;) {
        ReadLine(command, sizeof(command));
        fprintf(stderr, "RODENT: recv cmd='%s'\n", command);
...
void ParseGo(POS *p, const char *ptr) {
    ...
    fprintf(stderr, "RODENT: ParseGo entered\n");
    char token[80];
    ...
    Glob.isInfinite = infinite;

    fprintf(stderr, "RODENT: ParseGo moveTime=%d depth=%d nodes=%d infinite=%d\n",
            cEngine::msMoveTime, cEngine::msSearchDepth, cEngine::msMoveNodes, infinite);

    // get book move (skip in analysis/infinite mode)

    if (Par.useBook && !infinite) {
        ...
        if (pvb) {
            fprintf(stderr, "RODENT: book move found\n");
            printf("bestmove %s\n", MoveToStr(pvb));
            return;
        }
    }
    ...
    // In infinite mode, wait for "stop" before outputting bestmove.
    // The UCI spec requires that "go infinite" searches until "stop".
    if (infinite) {
        char waitCmd[4096];
        for (;;) {
            ReadLine(waitCmd, sizeof(waitCmd));
            fprintf(stderr, "RODENT: infinite wait recv='%s'\n", waitCmd);
            if (strcmp(waitCmd, "stop") == 0) break;
            if (strcmp(waitCmd, "quit") == 0) exit(0);
        }
    }
    ...
    fprintf(stderr, "RODENT: starting think threads (engines=%d)\n", (int)Engines.size());
    ...
    fprintf(stderr, "RODENT: search threads joined\n");
    ...
    fprintf(stderr, "RODENT: best_depth=%d, extracting move\n", best_depth);
    ExtractMove(best_pv);
#endif
