...
void PrintUciOptions() {
    ...
    if (Glob.usePersonalityFiles) {
        ...
        if (pers_aliases.count != 0) {
            // Find first real personality (skip "---" placeholder)
            const char *defPers = pers_aliases.alias[0];
            printf("option name Personality type combo default %s", defPers);
            for (int i = 0; i < pers_aliases.count; i++) {
                if (strcmp(pers_aliases.alias[i], "---") != 0)
