...
│
└── EngineServer/          # Desktop Java engine server
    └── src/main/java/org/petero/engineserver/
        ├── EngineServer.java   # Server manager
        ├── PortListener.java   # TCP listener + engine process
        ├── EngineConfig.java   # Configuration model
        ├── MainWindow.java     # Swing GUI
        └── ErrorHandler.java   # Error interface
```
...
### DroidFish Built-in EngineServer

The `EngineServer` module is a minimal Java desktop server:

- **Architecture**: One `PortListener` per engine, each on its own port
- **Config**: Java Properties file (`~/.engineServer.ini`)
- **GUI**: Optional Swing interface
- **Process model**: Spawns UCI engine as subprocess, raw-forwards stdin/stdout to TCP
- **Limitations**:
  - No security (any client can connect)
  - No logging
  - No connection management
  - Single client per engine
  - No UCI command inspection or modification
...
### DroidFish can also use its built-in EngineServer (Java):

```
┌─────────────┐          ┌──────────────────┐          ┌──────────────┐
│  DroidFish  │   TCP    │ EngineServer     │ stdin/   │ UCI Engine   │
│  Android    │◄────────►│ Java/Swing       │ stdout   │              │
│  App        │  UCI     │                  │◄────────►│              │
└─────────────┘          └──────────────────┘          └──────────────┘
```

The Chess-UCI-Server is a drop-in replacement for the built-in EngineServer, with significantly more features.
