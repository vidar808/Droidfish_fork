...
│   │       └── book/
│   │           └── LichessExplorerBookTest.java
│   └── src/androidTest/java/        # Instrumented tests (emulator)
│       └── org/petero/droidfish/
│           ├── ui/                  # Espresso UI tests (35 tests)
│           │   ├── BoardTestUtils.java
│           │   ├── DisableTourGuideRule.java
│           │   ├── MainActivityLaunchTest.java
│           │   ├── ChessBoardInteractionTest.java
│           │   ├── GameModeTest.java
│           │   ├── OpeningBookDisplayTest.java
│           │   ├── EngineAnalysisTest.java
│           │   ├── SettingsNavigationTest.java
│           │   └── ScreenshotCaptureTest.java
│           ├── gamelogic/           # Game logic tests (10 tests)
│           ├── book/                # Book tests (3 tests)
│           └── tb/                  # Tablebase tests (2 tests)
│
├── qa/                    # QA test infrastructure
│   ├── run_tests.sh           # Full test runner (emulator + build + test + screenshots)
│   ├── visual_diff.py         # Screenshot comparison tool (PIL-based)
│   ├── capture_and_validate.sh # Pull screenshots from device + compare
│   └── screenshots/           # baseline/, current/, diffs/
│
...
UCIEngine (interface)
  └── UCIEngineBase (abstract base)
       ├── InternalStockFish  - Bundled Stockfish via nativeLibraryDir
       ├── InternalRodent     - Bundled Rodent IV via nativeLibraryDir
       ├── InternalPatricia   - Bundled Patricia via nativeLibraryDir
       ├── ExternalEngine     - On-device executables (user places in DroidFish/uci/)
       │    └── ConfiguredEngine - Manifest-based engines (engine.json in subfolder)
       ├── NetworkEngine      - Remote engine via TCP socket ★
       └── OpenExchangeEngine - OEX protocol engines
```

Engine type detection order in `UCIEngineBase.getEngine()`:
1. Built-in engines (stockfish, rodent4, patricia)
2. Open Exchange engines (parent directory is "oex")
3. Network engines (NETE magic header)
4. Manifest-based engines (`engine.json` in directory)
5. Generic external engines (fallback for any executable)
...
| Tests | 60 JVM unit tests + 35 Espresso UI tests | 228 pytest tests |
